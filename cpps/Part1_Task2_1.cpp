...
int main() {
	...
	for(int tc = 1; tc <= 4; tc++) {
		for(int i = 1; i <= 5883; i++)
			de[i].clear();
		string tmpstring = "../data/results/Task2_1result" + to_string(tc) + ".txt";
		...
		for(nn = nn + 1; nn <= rn[tc]; nn++) {
			...
			for(list<onholdnode>::iterator it = onholdq.begin(); it != onholdq.end(); it++) {
				...
				currentroadpoint.u = it -> pathpoint == 0 ? it -> path[it ->  pathpoint] : it -> path[it ->  pathpoint - 1];
				currentroadpoint.v = it -> pathpoint >= (int)(it -> path.size()) ? it -> path[it ->  pathpoint - 1] : it -> path[it ->  pathpoint] ;
